...
import 'package:flutter_quill/src/easy4u/attribute_ext.dart';
import 'package:flutter_quill/src/widgets/style_widgets/bullet_image.dart';
...
class EditableTextBlock extends StatelessWidget {
  ...
  Widget? _buildLeading(
      BuildContext context, Line line, int index, Map<int, int> indentLevelCounts, int count) {
    final defaultStyles = QuillStyles.getStyles(context, false);
    final attrs = line.style.attributes;
    // print("buildLeading key:${AttributeExt.imageList.key} ${attrs} ${attrs.containsKey(AttributeExt.imageList.key)}");
    ...
    } else if (attrs.containsKey(AttributeExt.imageList.key)) {
      final imageAttribute = attrs[AttributeExt.imageList.key]!;
      final widget = AttributeExt.imageBulletBuilder?.call(imageAttribute.value.toString());
      // print("provider:$provider");
      if (widget != null) {
        return QuillBulletImage(
          image: widget,
          width: 32,
        );
      }
    ...
    if (attrs.containsKey(Attribute.header.key)) {
      ...
      if (attrs.containsKey(Attribute.blockQuote.key)) {
        ...
        lineSpacing = defaultStyles!.indent!.lineSpacing;
          ...
          attrs.containsKey(AttributeExt.imageList)) {
...
class RenderEditableTextBlock extends RenderEditableContainerBox implements RenderEditableBox {
  ...
  TextPosition? getPositionAbove(TextPosition position) {
    ...
    return TextPosition(
        offset: sibling.container.offset + sibling.getPositionForOffset(finalOffset).offset);
  }
  ...
  TextSelectionPoint getBaseEndpointForSelection(TextSelection selection) {
    ...
    final basePoint = baseChild!
        .getBaseEndpointForSelection(localSelection(baseChild.container, selection, true));
    return TextSelectionPoint(
        basePoint.point + (baseChild.parentData as BoxParentData).offset, basePoint.direction);
  }
  ...
  TextSelectionPoint getExtentEndpointForSelection(TextSelection selection) {
    ...
    final extentPoint = extentChild!
        .getExtentEndpointForSelection(localSelection(extentChild.container, selection, true));
    return TextSelectionPoint(extentPoint.point + (extentChild.parentData as BoxParentData).offset,
